...
| "Did you train anything?" | We built BKT parameter fitting. It produced a **negative** result on held-out data, so we did not adopt the fitted values and we report that as measured. The parameters we do ship were argued from the grading model rather than fitted — `p_guess` is computed per item from how falsifiable the test suite is. Same story with a retrieval reranker: recall@4 was already 100%, so we measured it and didn't build it. |
| "How do you know your own model was wrong?" | We measured it failing. The learning rate was applied after every observation including failures, which put an attracting floor at 6/35 = 0.171 — no student could be recorded below it however badly it went. `analytic_floor()` now states that in closed form and is unit-tested to still reproduce 6/35 for the old parameters. |
